...
```{r}
ggplot(penguins, aes(x = flipper_length_mm)) + #information about the dataset
  geom_histogram(binwidth=2, color = "magenta", fill="brown") + #make the figure
...
ggplot (penguins, aes(x=body_mass_g, fill = species)) +
        ...
        labs (x = "Body mass (g)", y = "Frequency") + 
        scale_fill_manual (values = c("springgreen", "limegreen", "forestgreen"))
```
...
ggplot (penguins, aes(x = bill_depth_mm, y = flipper_length_mm, color = body_mass_g,)) +
  geom_point (size = 7.777) +
  labs (x = "Bill depth (mm)", y = "Flipper length (mm)") +
  scale_color_gradient (low = "springgreen", high = "black")
  #scale_color_manual (values = c("springgreen", "turquoise"))
